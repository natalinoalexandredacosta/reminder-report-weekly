...
bot.command('status', async (ctx) => {
  const textForAdmin = await formatStatusList();
  const textForGroup = await formatStatusList({ withCc: true });
  // Kirim ke admin via DM (tanpa cc)
  await replyAdmin(ctx, textForAdmin, { parse_mode: 'Markdown' });
  // Kirim juga ke grup jika sudah diatur (dengan cc)
  const groupId = await getGroupId();
  if (groupId) {
    await ctx.telegram.sendMessage(groupId, textForGroup, { parse_mode: 'Markdown' });
...
bot.command('late', async (ctx) => {
  const textForAdmin = await formatLateList();
  const textForGroup = await formatLateList({ withCc: true });
  // Kirim ke admin via DM (tanpa cc)
  await replyAdmin(ctx, textForAdmin, { parse_mode: 'Markdown' });
  // Kirim juga ke grup jika sudah diatur (dengan cc)
  const groupId = await getGroupId();
