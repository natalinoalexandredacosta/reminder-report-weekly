...
bot.command('status', async (ctx) => {
  ...
  return replyAdmin(ctx, text, { parse_mode: 'Markdown' });
});
...
bot.command('uploaded', async (ctx) => {
  ...
  return replyAdmin(ctx, `✅ *${team.name}* ditandai sudah upload.`, { parse_mode: 'Markdown' });
});
...
bot.command('not_uploaded', async (ctx) => {
  ...
  return replyAdmin(ctx, `❌ *${team.name}* ditandai belum upload.`, { parse_mode: 'Markdown' });
});

bot.command('reset', async (ctx) => {
  await resetStatus();
  return replyAdmin(ctx, '🔄 Status minggu ini telah *direset*. Semua tim kembali ke status belum upload.', { parse_mode: 'Markdown' });
});
