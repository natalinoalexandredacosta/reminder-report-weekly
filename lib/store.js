...
const MEMBERS_KEY = 'group_members';
...
async function loadStatus() {
  ...
  try {
    ...
    // Jangan return getDefaultStatus() di sini, karena akan memicu overwrite
    // data kosong jika Upstash KV sedang sleep/timeout.
    throw new Error('Gagal memuat data dari KV: ' + err.message);
  }
}
...
// ─── Group Members (auto-captured dari pesan grup) ───────────────────────────

export async function getMembers() {
  const client = getKv();
  if (client) {
    try {
      const data = await client.get(MEMBERS_KEY);
      if (data) return data; // array of { id, username, first_name }
    } catch (err) {
      console.error('[KV] getMembers error:', err);
    }
  }
  if (!global.__membersCache) global.__membersCache = [];
  return global.__membersCache;
}
...
export async function upsertMember(user) {
  ...
  if (client) {
    try {
      await client.set(MEMBERS_KEY, members);
    } catch (err) {
      console.error('[KV] upsertMember error:', err);
    }
  } else {
    global.__membersCache = members;
  }
}
