...
function getKv() {
  ...
  const REST_URL = (process.env.KV_REST_API_URL || process.env.KV_URL_KV_REST_API_URL || process.env.KV_URL || '').trim();
  ...
  if (!isValidUrl || !isValidToken) {
    console.warn('[KV] Not configured. URL:', REST_URL, '| Token valid:', isValidToken);
    _kv = null;
    return _kv;
  }
